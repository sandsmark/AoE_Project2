...
UIElement::UIElement(bool argenabled, int argx, int argy, ElementType argtype, SDL_Texture *argtexture) :
    enabled(argenabled), pos(argx, argy), type(argtype), texture(argtexture)
{
...
// IMAGE
Image::Image(SDL_Rect argsection, int x, int y, SDL_Texture *argtexture) :
    UIElement(true, x, y, IMAGE, argtexture), section(argsection) {}
Image::Image(int x, int y, SDL_Texture *argtexture) :
    UIElement(true, x, y, IMAGE, argtexture) {}
...
Label::Label(char *text, int x, int y, _TTF_Font *font) :
    UIElement(true, x, y, LABEL, nullptr), str(text), font(font)
{
    color = { 0, 0, 0, 250 };
...
Label::Label(string text, int x, int y, _TTF_Font *font) :
    UIElement(true, x, y, LABEL, nullptr), str(text), font(font)
{
    color = { 0, 0, 0, 250 };
    texture = App->font->Print(str.c_str(), color, font);
...
Label::Label(char *text, SDL_Rect area, _TTF_Font *font) :
    UIElement(true, area.x, area.y, LABEL, nullptr), str(text), font(font)
{
    color = { 0, 0, 0, 250 };
    texture = App->font->Print(str.c_str(), color, font);
    App->font->CalcSize(str.c_str(), width, height, font);
...
Button::Button(int x, int y, vector<SDL_Rect> blit_sections, vector<SDL_Rect> detect_sections, ButtonTier Tier, SDL_Texture *argtexture) :
    UIElement(true, x, y, BUTTON, argtexture), button_tier(Tier)
{
...
InputText::InputText(int x, int y, SDL_Rect detect_area, _TTF_Font *font) :
    UIElement(true, x, y, INPUTTEXT, nullptr), font(font), area(detect_area)
{
    editable = false;
    str = "write here";
    bar.w = 2;
...
ScrollBar::ScrollBar(int x, int y, ScrollBarModel model) :
    UIElement(true, x, y, SCROLLBAR, nullptr), model(model)
{
    switch (model) {
    case MODEL1:
...
Quad::Quad(SDL_Rect area, SDL_Color color) :
    UIElement(true, area.x, area.y, QUAD, nullptr), color(color), area(area)
{
}
void Quad::CleanUp()
{
...
Cursor::Cursor(SDL_Texture *tex, vector<SDL_Rect> area) :
    UIElement(true, area[1].x, area[1].y, CURSOR, tex), sprite_list(area)
{
    id = 0;
}
