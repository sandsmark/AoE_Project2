...
Arrow::Arrow(int spd, pair<int, int> source, pair<int, int> dest) :
    src(source), dst(dest)
{
    //blits = anim.frames()
...
Emitter::Emitter(pair<float, float> startingposition, bool finite, float duration) :
    pos(startingposition), finite(finite), lifetime(duration)
{
}
