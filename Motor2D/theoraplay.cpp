...
{
    return pthread_create(&ctx->worker, nullptr, routine, ctx);
}
...
{
    return pthread_mutex_init(&ctx->lock, nullptr);
}
...
#define queue_ogg_page(ctx)                     \
    ...
    if (!ctx->halt && tpackets) {
        ...
        if ((tinfo.colorspace != TH_CS_UNSPECIFIED) && (tinfo.colorspace != TH_CS_ITU_REC_470M) && (tinfo.colorspace != TH_CS_ITU_REC_470BG)) {
...
#define VIDCVT(t)                           \
    case THEORAPLAY_VIDFMT_##t:             \
        vidcvt = ConvertVideoFrame420To##t; \
        break;
        VIDCVT(YV12)
        VIDCVT(IYUV)
#undef VIDCVT
    default:
        goto startdecode_failed; // invalid/unsupported format.
    } // switch
...
int THEORAPLAY_isDecoding(THEORAPLAY_Decoder *decoder)
{
    ...
    if (ctx) {
        Mutex_Lock(ctx->lock);
        retval = (ctx && (ctx->audiolist || ctx->videolist || (ctx->thread_created && !ctx->thread_done)));
        Mutex_Unlock(ctx->lock);
    } // if
    return retval;
